...
public class BlossomsPoGoManager {
	...
	public static void main(String[] args) throws Exception {
		...
		while (!logged) {
			...
			int response = JOptionPane.showConfirmDialog(null, panel, "Login", JOptionPane.YES_NO_CANCEL_OPTION,
			...
			if (response == JOptionPane.CANCEL_OPTION) {
				...
				try {
					PtcCredentialProvider provider = new PtcCredentialProvider(http, username.getText(),
							password.getText());
					cp = provider;
				...
				if (authCode == null) {
					...
					JOptionPane.showMessageDialog(null,
							...
							"Google Auth", JOptionPane.PLAIN_MESSAGE);
					...
					if (Desktop.isDesktopSupported()) {
						...
						if (JOptionPane.showConfirmDialog(null,
								...
								"Google Auth", JOptionPane.OK_CANCEL_OPTION,
					...
					authCode = JOptionPane.showInputDialog(null, "Please provide the authentication code",
							"Google Auth", JOptionPane.PLAIN_MESSAGE);
				} else {
					JOptionPane.showMessageDialog(null, "Logging in using cached google auth token!", "Google Auth",
							JOptionPane.PLAIN_MESSAGE);
					refresh = true;
				}
				try {
					...
					if (checkSaveAuth()) {
						if (!refresh)
							config.put("GoogleAuthToken", provider.getRefreshToken());
					} else {
						config.remove("GoogleAuthToken");
					}
				} catch (Exception e) {
	...
	private static void alertFailedLogin() {
		JOptionPane.showMessageDialog(null, "Unfortunately, your login has failed. Press OK to try again.",
	...
	private static boolean checkSaveAuth() {
		...
		return JOptionPane.showConfirmDialog(null,
				"Do you wish to save the password/auth token?\nCaution: These are saved in plain-text.",
				"Save Authentication?", JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE) == JOptionPane.OK_OPTION;
	}
}
