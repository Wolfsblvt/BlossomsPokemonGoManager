...
public class PokemonTab extends JPanel {
			...
			selection.forEach(poke -> {
				...
				try {
					...
					if (result == ReleasePokemonResponseOuterClass.ReleasePokemonResponse.Result.SUCCESS) {
						...
						System.out.println(
								"Transferring " + StringUtils.capitalize(poke.getPokemonId().toString().toLowerCase())
										+ ", Result: Success!");
						System.out.println(
						...
						System.out.println("Error transferring "
								+ StringUtils.capitalize(poke.getPokemonId().toString().toLowerCase()) + ", result: "
								+ result);
						err.increment();
					}
				} catch (Exception e) {
				...
				selection.forEach(poke -> {
					...
					try {
						...
						if (er.isSuccessful()) {
							...
							System.out.println(
									"Evolving " + StringUtils.capitalize(poke.getPokemonId().toString().toLowerCase())
											+ ". Evolve result: Success!");
							System.out.println("Stat changes: (Candies: " + newcandies + "[" + (newcandies - candies)
									+ "], CP: " + newcp + "[+" + (newcp - cp) + "], HP: " + newhp + "[+" + (newhp - hp)
									+ "])");
							success.increment();
						} else {
							err.increment();
							System.out.println(
									"Error evolving " + StringUtils.capitalize(poke.evolve().toString().toLowerCase())
											+ ", result: " + er);
						}
					} catch (Exception e) {
						err.increment();
	...
	private void powerUpSelected() {
		...
		if (selection.size() > 0) {
			if (confirmOperation("PowerUp", selection)) {
					...
					try {
						...
						if (result == UpgradePokemonResponseOuterClass.UpgradePokemonResponse.Result.SUCCESS) {
							...
							System.out.println("Powering Up "
									+ StringUtils.capitalize(poke.getPokemonId().toString().toLowerCase())
									+ ", Result: Success!");
							System.out.println("Stat changes: (Candies : " + newCandies + "[-" + (newCandies - candies)
									+ "], CP: " + newCp + "[+" + (newCp - cp) + "], HP: " + newHp + "[+" + (newHp - hp)
									+ "]) Stardust used " + stardustUsed + "[remaining: "
									+ go.getPlayerProfile().getCurrency(Currency.STARDUST) + "]");
							success.increment();
						} else {
							err.increment();
							System.out.println("Error powering up "
									+ StringUtils.capitalize(poke.getPokemonId().toString().toLowerCase())
									+ ", result: " + result);
						}
					} catch (Exception e) {
				...
				SwingUtilities.invokeLater(this::refreshList);
				JOptionPane.showMessageDialog(null,
						"Pokemon batch powerup complete!\nPokemon total: " + selection.size()
	...
	private boolean confirmOperation(String operation, ArrayList<Pokemon> pokes) {
		...
		pokes.forEach(p -> {
			String str = StringUtils.capitalize(p.getPokemonId() + "") + " - CP: " + p.getCp() + ", IV: "
					+ (Math.round(p.getIvRatio() * 10000) / 100) + "%";
			innerPanel.add(new JLabel(str));
		});
		panel.add(scroll);
		int response = JOptionPane.showConfirmDialog(null, panel,
				"Please confirm " + operation + " of " + pokes.size() + " pokemon", JOptionPane.OK_CANCEL_OPTION,
	...
	private void refreshList() {
		...
		go.getInventories().getPokebank().getPokemons().forEach(poke -> {
			String searchme = poke.getPokemonId() + "" + poke.getPokemonFamily() + poke.getNickname()
		...
		pt.constructNewTableModel(go, (search.equals("") || search.equals("searchpokemon...")
	...
	public static class PokemonTable extends JTable {
		 ...
		 * Type / Pokemon 3 Double - IV % 4 String - Type 1 5 String - Type 2 6
		 * String - Move 1 7 String - Move 2 8 Integer - CP 9 Integer - HP 10
		 * Integer - Candies of type 11 Integer - Candies to Evolve 12 Integer -
		 * Star Dust to level 13 String - Pokeball Type
		 */
		int sortColIndex = 1;
		...
		public void constructNewTableModel(PokemonGo go, List<Pokemon> pokes) {
			...
			Comparator<Integer> c = (i1, i2) -> {
				return Math.round(i1 - i2);
			};
			trs.setComparator(1, c);
			trs.setComparator(3, (d1, d2) -> {
	...
	public static class PokemonTableModel extends AbstractTableModel {
		...
		public PokemonTableModel(List<Pokemon> pokes, PokemonTable pt) {
			...
			pokes.forEach(p -> {
				...
				speciesCol.add(i.getValue(), StringUtils.capitalize(
						p.getPokemonId().toString().toLowerCase().replaceAll("_male", "♂").replaceAll("_female", "♀")));
				...
				type2Col.add(i.getValue(),
						StringUtils.capitalize(p.getMeta().getType2().toString().toLowerCase().replaceAll("none", "")));
				move1Col.add(i.getValue(), WordUtils.capitalize(
						p.getMove1().toString().toLowerCase().replaceAll("_fast", "").replaceAll("_", " ")));
				move2Col.add(i.getValue(),
				...
				pokeballCol.add(i.getValue(), WordUtils.capitalize(
